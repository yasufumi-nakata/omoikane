...
- `parallel_codex_post_commit_publication_receipt.schema`
  - commit-finalized execution receipt を `git push origin HEAD:refs/heads/main` と `git ls-remote origin refs/heads/main` の command receipt digest、local commit head、remote head、publication digest へ縮約し、origin/main 公開後の GitHub handoff 可否を raw execution / push / remote verification payload 非保存で固定する
