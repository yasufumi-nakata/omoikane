...
- `parallel_codex_integration_execution_receipt.schema`
  - integration-ready batch receipt を main checkout 適用直前の ordered apply plan、repo-local patch artifact manifest、`artifacts/parallel-codex/*.patch` path、command-bound git-apply-check dry-run receipt、post-apply verification manifest に縮約し、source batch digest、current head、apply step digest、raw batch / apply / dry-run / worker / verification payload 非保存を固定する
