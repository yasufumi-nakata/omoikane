...
- `parallel_codex_worker_result_receipt.schema`
  - parallel Codex worker / subagent / codex exec / remote branch / PR / Yaoyorozu dispatch result を main checkout 統合前に patch digest、changed file manifest digest、workspace marker hygiene digest、verification manifest digest、worker base commit freshness、signed worker identity evidence、remote metadata digest、remote source revocation digest、remote source revocation freshness digest、signed provider timestamp digest、timestamp replay guard digest、upstream dispatch / patch candidate digest、raw payload 非保存 flag へ縮約し、workspace-enacted marker-only result を blocked にする
