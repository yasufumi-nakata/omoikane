...
- `sensory_loopback_biodata_arbitration_binding.schema`
  - shared loopback の participant BioData confidence / feature-window drift gate / fresh calibration refresh receipt と hardware timing latency drift gate を同じ participant set / threshold authority digest に束縛し、3-4 participant の weighted latency quorum では participant weight / failed participant / quorum digest だけを保持して raw BioData / refresh / timing payload を保存しない
