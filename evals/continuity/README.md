...
### Memory Replication Quorum
MemoryCrystal manifest が four-target placement、Merkle audit、quorum-bound reconcile、
council escalation、identity/jurisdiction-bound media renewal cadence policy、
long-term media registry endpoint certificate CT/SCT evidence を通じて source digest を
保ったまま再同期要求を返せるか。
...
patch artifact cleanup digest、commit finalization digest に束縛し、さらに dry-run command を
...
checkout mutation attestation 未束縛 execution、patch cleanup 未 verified execution、
commit finalization gate 未 ready execution を fail-closed にできるか。
...
- `parallel_codex_post_commit_publication.yaml`
  - commit-finalized Parallel Codex handoff が pre-push origin/main freshness、push / remote-head verification、ls-remote output の observed head/ref digest、GitHub protected branch provider policy receipt、required verification checks、raw provider payload 非保存まで束縛すること
