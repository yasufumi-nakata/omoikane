...
- `self_model_care_trustee_registry_binding.yaml`
...
`self_model_care_trustee_registry_binding.yaml` は `self-model-demo` の
care-trustee-registry-binding branch と対応し、handoff で列挙された trustee /
...
`self_model_value_archive_retention_proof.yaml` は `self-model-demo` の
value-archive-retention-proof branch と対応し、retired value の archive snapshot refs が
