...
| `bdb-demo` / `biodata-transmitter-demo` / `neuro-integration-demo` / `collective-demo` | L6 interface の proxy contract、biosignal roundtrip、アンケート+EEG seed から fMRI / 脳オルガノイド expansion lane までの multi-application workbench、collective identity の bounded merge / recovery receipt | 生体データの実測、身体状態の医学的診断、意識再現、集合人格成立の証明 |
