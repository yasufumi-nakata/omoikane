...
source workspace に加えて bounded same-host local candidate workspace を `review_budget=3`
で走査した `yaoyorozu_workspace_discovery` を返し、
non-source workspace 群だけで runtime / schema / eval / docs の coverage が揃うことを確認する。
そのうえで、
...
target path observation と git-bound target path delta receipt を返すことを確認する。
