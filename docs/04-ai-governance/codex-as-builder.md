...
または structured patch-segment manifest digest、workspace marker hygiene digest、
...
digest、patch artifact cleanup digest、commit finalization digest を commit 前に固定する。
blocked batch、stale checkout head、dry-run failed execution、または patch artifact に
束縛されない dry-run command は blocked のまま保持し、
...
changed-file owner manifest に束縛されない execution、patch artifact cleanup が
`removed` として検証されない execution も fail-closed にする。
...
`parallel_codex_post_commit_publication_receipt` で local commit head、push 前 origin/main
...
bounded status/check poll budget digest、
...
timestamp、または status/check polling exhausted を GitHub handoff へ進めない。
...
polling / status check timestamp payload は保存しない。
