...
ewa.authorize(command, motor_plan, stop_signal_path, stop_signal_adapter_receipt, legal_execution) → ExternalActuationAuthorization
...
`legal_execution` / `guardian_oversight_gate` receipt が揃った時だけ発行される。
...
- `interface.ewa.v0.idl` は `prepare_motor_plan / prepare_stop_signal_path /
  probe_stop_signal_adapter / execute_legal_preflight / authorize / command /
  emergency_stop / observe / release`
  を machine-readable に固定
- `ewa_command.schema` / `ewa_motor_plan.schema` / `ewa_stop_signal_path.schema` /
  `ewa_stop_signal_adapter_receipt.schema` / `ewa_legal_execution.schema` /
...
- `ewa-demo` を CLI に追加し、motor plan → stop-signal path arming →
  PLC / firmware adapter readiness probe → legal preflight →
...
- 実機 vendor API / safety PLC への production connector 認証と設置証明
- 実 regulator / permit API との接続 → [../../05-research-frontiers/governance.md](../../05-research-frontiers/governance.md)
