...
ewa.authorize(command, motor_plan, legal_execution) → ExternalActuationAuthorization
...
matching `motor_plan` / `legal_execution` receipt が揃った時だけ発行される。
...
- `interface.ewa.v0.idl` は `prepare_motor_plan / execute_legal_preflight /
  authorize / command / emergency_stop / observe / release` を machine-readable に固定
- `ewa_command.schema` / `ewa_motor_plan.schema` / `ewa_legal_execution.schema` /
  `ewa_audit.schema` / `external_actuation_authorization.schema` /
  `ewa_emergency_stop.schema` を導入
- `ewa-demo` を CLI に追加し、motor plan → legal preflight → authorize(reversible) →
  command(reversible) →
