...
5. **release は必ず実行** ── timeout / 緊急 disconnect は強制 release
6. **要約のみ ledger** ── 機微なセンサデータは hash で参照、生データは別 store

## reference runtime の扱い

- `interface.ewa.v0.idl` を導入し、`acquire / authorize / command / observe / release` の 5 op
- `ewa_command.schema` / `ewa_audit.schema` / `external_actuation_authorization.schema` を導入
- `ewa-demo` を CLI に追加し、acquire → authorize(reversible) → command(reversible) → veto(irreversible 試行) → release を実行
