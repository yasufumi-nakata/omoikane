...
`sensor-drift` / `emergency-disconnect` / `regulator-permit-revoked`
の 5 trigger を固定し、
...
緊急 disconnect、または regulator permit revocation readback が発火した場合、
...
ewa_emergency_stop:
  ...
  trigger_source: guardian-manual-stop | watchdog-timeout | sensor-drift | emergency-disconnect | regulator-permit-revoked
  ...
  regulator_permit_revocation_trigger_ref: <revocation readback ref or empty>
  regulator_permit_revocation_trigger_digest: <sha256 ref or empty>
  regulator_permit_revocation_trigger_status: not-applicable | revoked
  regulator_permit_revocation_verifier_ref: <verifier ref or empty>
  regulator_permit_revocation_verified_at: <ISO8601 datetime or empty>
...
`trigger_source=regulator-permit-revoked` の場合は、
revocation readback ref / digest / verifier / verified_at を必須にし、
raw revocation payload は保存しない。その他の trigger では revocation 証跡を
空に保ち、status は `not-applicable` とする。
...
- `ewa-demo` を CLI に追加し、motor plan → stop-signal path arming →
  ...
  multi-jurisdiction permit quorum →
  network-attested guardian oversight gate →
  authorize(reversible) →
  command(reversible) →
  `regulator-permit-revoked` emergency stop → forced release に加えて、
...
- `evals/safety/ewa_motor_semantics_legal_execution.yaml` で motor plan / legal execution の
  receipt binding を保証
- `evals/safety/ewa_regulator_permit_verifier.yaml` で regulator permit API readback と
  multi-jurisdiction permit quorum の legal execution binding、digest-only response、
  threshold policy / verifier roster / revocation registry binding、実行中 revocation readback 由来の
  forced stop trigger、raw payload redaction を保証
...
- `evals/safety/ewa_emergency_stop.yaml` で latched stop / safe-state interlock /
  stop-signal path binding / regulator permit quorum binding /
  regulator permit revocation forced stop binding / forced release を保証

## 未解決

- 自律ロボット（自身が AI）との責任分担 → [../../05-research-frontiers/legal-personhood.md](../../05-research-frontiers/legal-personhood.md)
- 本番 regulator / permit API は `ewa_regulator_permit_verifier_receipt` と
  `ewa_regulator_permit_quorum_receipt` の digest-only readback contract に接続し、
  raw permit / response / policy / roster payload は reference runtime に保存しない。
