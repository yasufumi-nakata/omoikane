...
- raw questionnaire / EEG / neuroimaging / organoid / analysis / connector / credential /
  endpoint / collection payload は保存しない
...
12. 現在の source type 全ペアに survey+EEG alignment、EEG+fMRI context、
    organoid context、generic feature-summary screen の bounded recipe を割り当てる
13. 各 pair の bounded result summary を生成し、operator と coding agent の review-ready
    receipt として束縛する
14. ContinuityLedger に upstream receipt、source bundle、workspace、analysis、guide、
    replacement plan、connector bundle、collection protocol、cross-modal analysis plan、
    analysis run を記録する
...
9. **cross-modal pair coverage** ── 現在の source type 全ペアは bounded recipe と 5 lane connector support を持つ
10. **bounded result summaries** ── result は pair digest と bounded axis summary に限り、診断や因果推論には昇格しない
11. **digest-only** ── raw source / raw app / raw analysis / raw connector / raw collection / raw plan / raw result payload を保存しない
12. **claim ceiling** ── clinical diagnosis、consciousness reproduction、identity replacement はすべて false のまま維持する
