...
- shared session の途中で refresh state を recheck する場合は、
  expired / revoked / stale refresh を fail-closed として delivery block、
  shared session hold、safe baseline requirement へ伝播する
...
sensory_loopback.bind_latency_weight_policy_verifier_quorum:
  input:
    authority_ref: <weighted latency policy authority ref>
    authority_digest: <weighted latency policy authority digest>
...
sensory_loopback.bind_participant_calibration_refresh_state_guard:
  input:
    biodata_arbitration_binding: <sensory_loopback_biodata_arbitration_binding>
    participant_refresh_states:
      <identity ref>: fresh | expired | revoked | stale
    participant_revocation_refs:
      <identity ref>: <revocation registry ref, revoked only>
    observed_at_ref: <timestamp/evidence ref>
  output: sensory_loopback_calibration_refresh_state_guard
...
`bind_participant_calibration_refresh_state_guard` は既存の shared arbitration binding を
runtime 中に再点検するための guard である。`participant_refresh_states` は binding の
`participant_identity_ids` と完全一致しなければならず、各 participant の元の
refresh ref / digest / source digest set / window flag を再利用する。
`expired`、`revoked`、`stale` のいずれかが含まれる場合、
`refresh_fail_closed=true`、`delivery_blocked=true`、
`shared_session_hold_required=true`、`safe_baseline_required=true` になる。
`revoked` は revocation registry ref を必須とし、それ以外の status は revocation ref を
持たない。guard は refresh state digest set、guard digest、revocation ref だけを保持し、
raw refresh / revocation payload は保存しない。
...
10. **refresh state は fail-closed** ── mid-session recheck で expired / revoked / stale refresh が出た時は delivery block、shared session hold、safe baseline requirement を同時に立て、raw revocation payload を保持しない

## reference runtime の扱い

- `interface.sensory_loopback.v0.idl` を追加し、
  `open_session / bind_latency_weight_policy_verifier_quorum / bind_participant_biodata_arbitration / bind_participant_calibration_refresh_state_guard / deliver_bundle / stabilize / snapshot / capture_artifact_family / snapshot_artifact_family` を定義
- `sensory_loopback_session.schema` /
  ...
  `sensory_loopback_calibration_refresh_state_guard.schema` を追加
...
- 同じ weighted path は peer の expired refresh と observer の revoked refresh を
  ...
  revocation payload redaction を検証する
- 同じ demo は `schema_contracts` manifest で self-only / shared の
  session、receipt、artifact family、BioData arbitration binding、refresh state guard を public schema path へ束縛し、
...
- `evals/interface/sensory_loopback_calibration_refresh_state_guard.yaml` で
  mid-session refresh state fail-closed、delivery block、shared session hold、
  guard digest、revocation ref、raw refresh / revocation payload redaction を固定
