...
`kernel.energy_budget.v0` は `EnergyBudgetService.evaluate_floor`、`evaluate_pool_floor`、`evaluate_voluntary_subsidy` と各 validation contract を公開する。
...
`EnergyBudgetService.evaluate_voluntary_subsidy` は pool floor receipt の validation 後にだけ、post-floor voluntary consent として別 identity の donor surplus を recipient shortfall に紐づける。`energy-budget-subsidy-demo` は council member の floor-preserved surplus `14 J/s` のうち `8 J/s` を migration member の shortfall `8 J/s` に明示同意 digest、revocation ref、funding policy ref、signature ref 付きで束縛し、`voluntary_subsidy_allowed=true`、`floor_protection_preserved=true`、`cross_identity_offset_used=false`、`raw_funding_payload_stored=false` を返す。
...
6. sustainable economy の社会制度モデルは research frontier に残し、runtime は床値保護と同意付き補助 receipt だけを検証する
