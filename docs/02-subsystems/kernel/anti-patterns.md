...
→ reference runtime では `kernel.energy_budget.v0` / `energy-budget-demo` が
  `requested_budget_jps < EnergyFloor` を `floor-protected` として拒否し、
  observed capacity が floor 未満なら SubstrateBroker の `migrate-standby`
  signal に receipt を束縛する。
→ multi-identity pool でも aggregate budget だけを見てはいけない。
  `energy-budget-pool-demo` は別 identity の surplus で pressured identity の
  floor 未満 request を相殺せず、`cross_identity_floor_offset_blocked=true`
  として per-identity floor を守る。
