...
`worker result -> signed worker identity evidence + repo-local workspace marker diff classifier + remote metadata + remote source revocation evidence + freshness window + signed provider timestamp + replay guard + remote content identity + remote ancestry + patch digest + changed files + verification result -> schema-bound ingestion receipt`
...
ordered apply step digest、pre-apply dry-run manifest、post-apply verification manifest を持つ
`parallel_codex_integration_execution_receipt` へ縮約し、conflict batch 由来の
execution や dry-run failed execution は blocked のまま commit へ進めない。
