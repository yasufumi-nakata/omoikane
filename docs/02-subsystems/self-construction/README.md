...
- `RollbackEngine` ── staged rollout の rollback 判定を受け、pre-apply snapshot 復元・通知・append-only continuity 記録に加え、live enactment に対応する reverse-apply journal と telemetry gate を実行
  ...
  current checkout baseline と照合する repo-bound verification receipt も gate に束縛する
...
`regression detect -> live enactment receipt bind -> canary rollback -> pre-apply snapshot restore -> actual reverse-apply command -> repo-bound verification -> reverse-apply journal -> stakeholder notify`
