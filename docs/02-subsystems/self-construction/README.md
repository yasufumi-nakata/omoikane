...
- `DesignReader` ── docs/ と specs/ を解釈し、`design_delta_manifest` と build_request handoff を生成
...
- `RollbackEngine` ── staged rollout の rollback 判定を受け、pre-apply snapshot 復元・通知・append-only continuity 記録に加え、live enactment に対応する reverse-apply journal と telemetry gate を実行
  ...
  detached git worktree 上で baseline 状態へ戻る checkout-bound mutation receipt と、
...
`regression detect -> live enactment receipt bind -> canary rollback -> pre-apply snapshot restore -> actual reverse-apply command -> repo-bound verification -> checkout-bound mutation restore -> external observer receipt -> reverse-apply journal -> stakeholder notify`
...
`docs/specs -> source digest -> design_delta_manifest -> build_request`
