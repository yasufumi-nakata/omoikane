...
`worker result -> signed worker identity evidence + remote metadata + remote source revocation evidence + freshness window + signed provider timestamp + patch digest + changed files + verification result -> schema-bound ingestion receipt`
