...
`worker result -> signed worker identity evidence + repo-local workspace marker diff classifier or structured patch-segment manifest + remote metadata + remote source revocation evidence + freshness window + signed provider timestamp + replay guard + remote content identity + remote ancestry + patch digest + changed files + verification result -> schema-bound ingestion receipt`
...
`upstream_binding_digest` へ縮約し、同じ worker identity evidence を束縛してから
Parallel Codex ingestion receipt に変換する。
さらに integration-ready batch を source batch digest、current checkout head、
ordered apply step digest、repo-local patch artifact manifest digest、
`artifacts/parallel-codex/*.patch` path、command-bound `git apply --check`
pre-apply dry-run manifest、post-apply verification manifest を持つ
`parallel_codex_integration_execution_receipt` へ縮約し、conflict batch 由来の
execution、dry-run failed execution、patch artifact に束縛されない dry-run command、
apply plan / patch artifact manifest / pre-apply dry-run manifest に束縛されない
post-apply verification、checkout mutation attestation が pre-apply head / post-apply head /
apply plan / patch artifact manifest / dry-run manifest / verification context /
changed-file owner manifest に束縛されない execution、commit finalization gate が
...
push 前 origin/main head、command-bound pre-push `git ls-remote origin refs/heads/main`
freshness receipt、origin/main remote head、command-bound `git push origin HEAD:refs/heads/main`
receipt、command-bound post-push `git ls-remote origin refs/heads/main` receipt を
...
provider timestamp が `signed-current` かつ replay guard が `unique` であることに加え、
`post-push-status-check-poll-budget-v1` が attempt count、max attempts、poll interval、
terminal status、suite / freshness digest を束縛し、terminal status が `completed` で
あることも要求する。
...
stale / invalid、suite timestamp replay guard が replayed、status/check polling が
budget 内で `completed` に到達しない、または source execution / commit finalization が未 ready の場合は schema-bound のまま
