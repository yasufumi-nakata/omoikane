...
- `DesignReader` ── docs/ と specs/ を解釈し、git-bound `design_delta_scan_receipt` と `design_delta_manifest` / build_request handoff を生成
- `PatchGenerator` ── `build_request` を読み、immutable boundary と workspace scope を検証したうえで target subsystem / output path に整列した patch descriptor を生成（Codex 系）
...
- `DifferentialEvaluator` ── `must_pass` eval を選定し、parsed baseline/sandbox evidence と comparison digest に基づく差分評価と promote/hold/rollback 判定を返す
...
`build_request -> target-aware patch descriptor -> sandbox apply -> parsed A/B eval report -> rollout classify -> staged rollout`
...
`docs/specs -> git HEAD delta scan -> source digest -> design_delta_manifest -> build_request`
