...
- manifest 内の `catalog_inventory_receipt` は `specs-catalog-generated-inventory-v1` として
  `specs/catalog.yaml` の entries、priority/kind counts、実装済み schema/IDL coverage、
  欠落・重複ゼロの検証結果を digest-bound に固定する
