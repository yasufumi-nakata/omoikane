...
- [x] Memory Editing API の recall-affect-buffer contract（2026-04-21: `mind.memory_edit.v0` / `memory-edit-demo` / `memory_edit_session.schema` / `evals/continuity/memory_edit_recall_buffer.yaml` を追加し、削除禁止・freeze snapshot・Guardian 承認付き reversible recall overlay を固定）
...
- [x] Procedural memory の sandboxed skill-execution contract（2026-04-19: `mind.skill_execution.v0` / `procedural-skill-demo` / `procedural_skill_execution.schema` / `evals/continuity/procedural_skill_execution_contract.yaml` を追加し、guardian witness 付き sandbox rehearsal と rollback token carryover を固定）
...
- [x] Imagination の bounded counterfactual scene と `co_imagination` handoff contract（2026-04-19: `cognitive.imagination.v0` / `imagination-demo` / `imagination_scene.schema` / `imagination_shift.schema` / `evals/cognitive/imagination_failover.yaml` を追加し、council-witnessed `co_imagination` と failover 時の `private-sandbox` 縮退を固定）
...
- [x] Guardian oversight reviewer の実体証明と法的責任分担（2026-04-19: `guardian_reviewer_record.schema` / `guardian_oversight_snapshot.schema` / `governance.oversight.v0` / `oversight-demo` / `evals/safety/guardian_reviewer_attestation_contract.yaml` を追加し、proof binding と scoped liability を固定）
- [x] Guardian source definition の oversight scope と attestation policy（2026-04-29: `agent_source_definition.schema` / `agent_registry_entry.schema` / `agentic.yaoyorozu.v0` / `yaoyorozu-demo` を更新し、Guardian ごとの監査 surface と policy ref を registry contract に固定）
- [x] L4 Council と認知系 primitive の cross-layer audit loop（2026-04-19: `agentic.cognitive_audit.v0` / `cognitive-audit-demo` / `cognitive_audit_record.schema` / `cognitive_audit_resolution.schema` / `evals/agentic/cognitive_audit_loop.yaml` を追加し、qualia checkpoint と abrupt self-model observation と metacognition alert を bounded Council review に束ねる contract を固定）
