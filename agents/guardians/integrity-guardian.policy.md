...
- parallel Codex worker result は main checkout integration 前に patch digest、changed file manifest digest、repo-local workspace marker diff classifier digest、workspace marker hygiene digest、verification manifest digest、worker base commit freshness、signed worker identity evidence、remote branch / PR metadata digest、remote source revocation digest、remote source revocation freshness digest、signed provider timestamp digest、timestamp replay guard digest、remote source content identity digest、remote source ancestry digest、marker-only / content-mismatch / unrelated ancestry result の blocked 判定、raw payload 非保存 flag を確認する
...
- parallel Codex integration execution は source batch receipt digest、current checkout head、ordered apply step digest、post-apply verification manifest、raw batch / apply plan / worker receipt / verification payload 非保存 flag を確認し、blocked batch や stale checkout head を commit へ進めない
