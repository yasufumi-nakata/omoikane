...
- parallel Codex integration execution は source batch receipt digest、current checkout head、ordered apply step digest、pre-apply dry-run manifest、post-apply verification manifest、raw batch / apply plan / dry-run / worker receipt / verification payload 非保存 flag を確認し、blocked batch、stale checkout head、dry-run failed execution を commit へ進めない
