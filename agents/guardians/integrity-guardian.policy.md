...
- parallel Codex post-commit publication は source execution receipt digest、commit finalization ready 状態、local commit head、origin/main remote head、command-bound `git push origin HEAD:refs/heads/main` receipt、command-bound `git ls-remote origin refs/heads/main` receipt、publication digest、raw execution / push / remote verification payload 非保存 flag を確認し、remote head mismatch や push / remote verification failure を GitHub handoff へ進めない
