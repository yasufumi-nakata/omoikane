...
- parallel Codex worker result は main checkout integration 前に patch digest、changed file manifest digest、workspace marker hygiene digest、verification manifest digest、worker base commit freshness、signed worker identity evidence、remote branch / PR metadata digest、remote source revocation digest、remote source revocation freshness digest、signed provider timestamp digest、timestamp replay guard digest、remote source content identity digest、marker-only / content-mismatch result の blocked 判定、raw payload 非保存 flag を確認する
