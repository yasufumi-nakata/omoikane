...
- parallel Codex integration batch は ordered integration digest、changed-file owner manifest digest、conflict digest、blocked receipt quarantine、required verification manifest、raw worker receipt / raw conflict payload 非保存 flag を確認し、changed-file overlap が残る batch を blocked のまま保持する
